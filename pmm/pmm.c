...
static struct Memory *openMemory(u32 phyAddr, size_t size)
{
	...
	mem->base = mmap(NULL, mem->size, PROT_READ | PROT_WRITE,
			 MAP_SHARED, mem->fd, mem->phyBase);
...
static void getReg(struct Memory *mem,
		   unsigned long addr, void *value, int unitSize)
{
	ASSERT(mem);
	...
	switch (unitSize) {
		case 1:
			*(u8 *)value =
				*(volatile u8 *)(((u8 *)mem->base)
						 + addr - mem->phyBase);
			break;
		case 2:
			ASSERT((addr & 1) == 0);
			*(u16 *)value =
				*(volatile u16 *)(((u8 *)mem->base)
						  + addr - mem->phyBase);
			break;
		case 4:
			ASSERT((addr & 3) == 0);
			*(volatile u32 *)value =
				*(volatile u32 *)(((u8 *)mem->base)
						  + addr - mem->phyBase);
			break;
		default:
        ...
        struct Memory *mem, unsigned long addr, void *value, int unitSize)
{
	...
	switch (unitSize) {
		case 1:
			*(volatile u8 *)(((u8 *)mem->base)
					 + addr - mem->phyBase) = *(u8 *)value;
			break;
		case 2:
			ASSERT((addr & 1) == 0);
			*(volatile u16 *)(((u8 *)mem->base)
					  + addr - mem->phyBase) = *(u16 *)value;
			break;
		case 4:
			ASSERT((addr & 3) == 0);
			*(volatile u32 *)(((u8 *)mem->base)
					  + addr - mem->phyBase) = *(u32 *)value;
			break;
		default:
			ASSERT(! "Never reached");
			break;
